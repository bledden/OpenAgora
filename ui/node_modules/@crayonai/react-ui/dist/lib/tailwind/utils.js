...
export const extractComponentsAndPaths = () => {
    ...
    const promises = components
        ...
        const componentStylePath = componentFiles.find((file) => file === `${camelCase(component)}.css`);
        let dependencies = [];
...
export const getComponentsDependencies = async (componentNames, componentDependencyMap) => {
    ...
    if (!componentsAndPaths)
        throw new Error("Failed to extract components and paths");
    const components = componentsAndPaths.filter((component) => componentNames.includes(component.name));
    if (!components)
        throw new Error(`Components ${componentNames.join(", ")} not found`);

...
const generateTailwindPlugin = async () => {
    ...
    let pluginFileContents = replacePlaceholderWithContent(templateFileContents, "COMPONENT_IMPORTS", "");
    pluginFileContents = replacePlaceholderWithContent(pluginFileContents, "COMPONENT_TYPES", componentTypes);
    pluginFileContents = replacePlaceholderWithContent(pluginFileContents, "ADD_COMPONENTS_CALLS", addComponentCalls);
    pluginFileContents = replacePlaceholderWithContent(pluginFileContents, "INCLUDE_ALL_COMPONENTS", includeAllComponents.toString());
    pluginFileContents = replacePlaceholderWithContent(pluginFileContents, "COMPONENT_DEPENDENCY_MAP", JSON.stringify(componentDependencyMap));
...
const generateIncludeAllComponents = async () => {
    const componentsAndPaths = await extractComponentsAndPaths();
    if (!componentsAndPaths)
        return "[]";
    const includeAllComponents = new Set(); // components which are dependent on all other components
...
const generateAddComponentsCalls = async () => {
    ...
    const addComponentsCallPromises = componentsAndPaths.map(async (component) => {
        if (UNSTYLED_COMPONENTS.has(component.name))
            return;
        if (!component.stylePath)
            throw new Error(`No styles found for component: ${component.name}. If this is intentional, add component name to UNSTYLED_COMPONENTS in generateTailwindPlugin.ts`);
...
const formatPluginFile = (pluginFileContents) => {
...
const replacePlaceholderWithContent = (templateFileContents, placeholderName, content) => {
...
const replaceNullWithObject = (obj) => {
...
const convertNumericValuesToStrings = (obj) => {
    return Object.fromEntries(Object.entries(obj).map(([key, value]) => {
...
const transformCssToJson = async (css) => {
...
//# sourceMappingURL=generateTailwindPlugin.js.map